...
/*	//freopen("PyTest.Log", "w", stdout);
	...
	//							"Error.Redirection()\n");
		...
		for(sleep = 0; sleep < 100; sleep ++){
			printf("[UDPProcess]: Check 1\n");
		}
    //udp module not loaded yet!
    //PyObject *pUDPMod;
	pUDPMod = PyImport_ImportModule("udp");//load udp.py
    if(!pUDPMod)
    {
    	printf("udp module not loaded yet!");
    	return 0;
    }    	
    verbose(2, "Executing Python scritps...\n");
   
    //printf("%s",pUDPMod);
    printf("udp Module loaded in ip\n");
    if (pUDPMod)
    {
        printf("module [udp] imported\n");
    	pUDPGlobalDict = PyModule_GetDict(pUDPMod);   // Get main dictionary
        CheckPythonError();
        if (pUDPGlobalDict)
        {
            printf("main dictionary got\n");
            pFunc = PyDict_GetItemString(pUDPGlobalDict, "UDPPacketProcess");//find func SetAdd
            CheckPythonError();
            if (pFunc)
            {
                printf("found function [UDPPacketProcess]\n");
                //pArg = SWIG_NewPointerObj((void *)&Object, SWIGTYPE_p__gpacket_t, 1);
                pArg = SWIG_NewPointerObj((void *)in_pkt, SWIGTYPE_p__gpacket_t, 1);
                pPcore = SWIG_NewPointerObj((void *)pcore, SWIGTYPE_p_pktcore_t, 1);
                printf("<----[UDPProcess_C]---pPcore@%d , pcore@%d--------->\n", pPcore, pcore);
                CheckPythonError();
                if(pArg)
                {
                    printf("Got Pyton obj\n");
                    pResult = PyObject_CallFunction(pFunc, "OO", pArg, pPcore);
                    CheckPythonError();
                    printf("pResult: %s",pResult);
                }
            }
...
int IPOutgoingPacket(gpacket_t *pkt, uchar *dst_ip, int size, int newflag, int src_prot)
{
	...
	} else if (newflag == 1)
	{
		...
		if(pkt == NULL)
				printf("pkt is null!!!\n");
			// for(i = 0; i < 30; i ++){
			// 	printf("[IPOutgoingPacket] Check point 2!\n");
			// }
			printf("=====Packet======");
...
int IPSend2Output(gpacket_t *pkt)
{
	...
	int i;
	// for(i = 0; i < 30; i ++){
	 	printf("[IPSend2Output] Check point 1!\n");
	// }
	...
	// 	if(pkt == NULL)
	// 		printf("pkt is null!!!!!!!!!!!!!!!!!");
	printf("[IPSend2Output] Check point 2! size of pkt : %d\n", sizeof(*pkt));
	// }
	// for(i = 0; i < 30; i ++)
	// {
	// 	if(pcore == NULL)
